...
func TestIdentifierExpression(t *testing.T) {
	...
	if !ok {
		t.Fatalf("stmt not *ast.ExpressionStatement, got=%q", stmt)
	}

	ident, ok := stmt.Expression.(*ast.Identifier)
	if !ok {
		t.Errorf("expression not *ast.Identifier, got=%T", stmt.Expression)
	}

	if ident.Value != "foobar" {
		t.Errorf("ident.Value not 'foobar', got=%q", ident)
	}
